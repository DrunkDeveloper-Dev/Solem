...
export default function LaunchDetailPage() {
  ...
  const [launchWalletSol, setLaunchWalletSol] = useState<number | null>(null);
  const [launchWalletSolLoading, setLaunchWalletSolLoading] = useState<boolean>(false);
  const pumpScanTriedRef = useRef(false);
  const withdrawTriggeredRef = useRef(false);
  const launchJobTriggeredRef = useRef(false);
    ...
    async function fetchStatus() {
      try {
        ...
        if (r.status === 200) {
          ...
          // Ensure background job runs even if server dispatch failed
          if (!launchJobTriggeredRef.current && latest?.status && String(latest.status).startsWith('deposit')) {
            launchJobTriggeredRef.current = true;
            try { await axios.post('/api/jobs/launch', { id }); } catch {}
          }
  ...
  const refreshLaunchWalletSolBalance = useCallback(async () => {
    const launchAddress = data?.launchWallet;
    if (!launchAddress) return null;
    setLaunchWalletSolLoading(true);
    try {
      const lamports = await connection.getBalance(new PublicKey(launchAddress));
      const sol = lamports / LAMPORTS_PER_SOL;
      setLaunchWalletSol(sol);
      return sol;
    } catch {
      return null;
    } finally {
      setLaunchWalletSolLoading(false);
    }
  }, [connection, data?.launchWallet]);

  useEffect(() => {
    if (!data?.launchWallet) return;
    refreshLaunchWalletSolBalance();
  }, [data?.launchWallet, refreshLaunchWalletSolBalance]);
  ...
  async function triggerWithdraw() {
    ...
    try {
      ...
      try { await refreshLaunchWalletSolBalance(); } catch {}
    } finally {
      setBusy(null);
    }
  }
  ...
  async function withdrawToPlatformWallet() {
    if (!id) return;
    setBusy('Withdrawing to platform');
    show('Starting withdraw to platform wallet…', 'info');
    try {
      const r = await axios.post('/api/jobs/reverse-privacy', { id });
      if (r.status !== 200) {
        let msg = 'Withdraw to platform failed';
        try {
          const j = r.data;
      ...
      const j = r.data;
      const amount = typeof j?.depositedLamports === 'number' ? (j.depositedLamports / LAMPORTS_PER_SOL).toFixed(4) + ' SOL' : undefined;
      show(j?.ok ? `Withdrawn ${amount ?? 'funds'} to platform wallet` : 'Withdraw to platform requested', 'success');
      try {
        const st = await axios.get(`/api/launch/${id}/status`);
        if (st.status === 200) setData(st.data);
      } catch {}
      ...
      console.error(e);
      show('Withdraw to platform failed', 'error');
    } finally {
      setBusy(null);
    }
  }
      ...
      <div className="card space-y-2 text-sm">
        ...
        {data.launchWallet && (
          <div className="text-xs text-zinc-400">
            Launch wallet SOL: {launchWalletSolLoading ? 'Loading…' : (launchWalletSol !== null ? `${launchWalletSol.toLocaleString(undefined, { maximumFractionDigits: 6 })} SOL` : '—')}
            <button onClick={refreshLaunchWalletSolBalance} className="ml-2 rounded-md bg-white/10 px-2 py-1 text-xs hover:bg-white/20">Refresh</button>
          </div>
        )}
        <div className="flex flex-wrap gap-2 pt-2">
          ...
          {data?.pump?.mint && (
            <>
              ...
              <button className="btn" disabled={!!busy} onClick={withdrawToPlatformWallet}>Withdraw to platform wallet</button>
            </>
          )}
        </div>
        {data.pump?.mint && (
